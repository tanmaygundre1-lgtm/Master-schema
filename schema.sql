-- ============================================================================
-- School ERP Unified Schema (PostgreSQL 15+)
-- Admission-first architecture with fee module integration
-- ============================================================================
-- Notes:
-- 1) Admission is the master reference for all fee transactions.
-- 2) No duplicate CREATE TABLE / CREATE INDEX blocks.
-- 3) Circular dependency solved by adding admission.application_id after application exists.
-- 4) Sample seed data removed for production cleanliness.
-- ============================================================================
...
DROP TABLE IF EXISTS application_academic_info CASCADE;

DROP TABLE IF EXISTS application_parent_info CASCADE;

DROP TABLE IF EXISTS application_student_info CASCADE;
...
DROP TABLE IF EXISTS campus_visit CASCADE;
...
-- ============================================================================
-- Admission (Master for Fees) - Created without application FK
-- ============================================================================
...
-- Circular dependency fix (single definition, no duplicate index/alter)
...
-- ============================================================================
-- Timestamp and Status Trigger Functions
-- ============================================================================
...
CREATE TRIGGER tr_trigger_update_app_on_student_info
AFTER INSERT OR UPDATE ON application_student_info
FOR EACH ROW EXECUTE FUNCTION update_application_timestamp();

CREATE TRIGGER tr_trigger_update_app_on_parent_info
AFTER INSERT OR UPDATE ON application_parent_info
FOR EACH ROW EXECUTE FUNCTION update_application_timestamp();

CREATE TRIGGER tr_trigger_update_app_on_academic_info
AFTER INSERT OR UPDATE ON application_academic_info
FOR EACH ROW EXECUTE FUNCTION update_application_timestamp();

CREATE TRIGGER tr_fee_structure_updated_at
BEFORE UPDATE ON fee_structure
FOR EACH ROW EXECUTE FUNCTION update_timestamp();

CREATE TRIGGER tr_student_fee_assignment_updated_at
BEFORE UPDATE ON student_fee_assignment
FOR EACH ROW EXECUTE FUNCTION update_timestamp();

CREATE TRIGGER tr_invoice_updated_at
...
CREATE TRIGGER tr_lead_activity_updated_at
BEFORE UPDATE ON lead_activity
FOR EACH ROW EXECUTE FUNCTION update_timestamp();

CREATE TRIGGER tr_campus_visit_updated_at
BEFORE UPDATE ON campus_visit
FOR EACH ROW EXECUTE FUNCTION update_timestamp();
